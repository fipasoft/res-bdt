    ...
    <tr>
      <td>Fecha de ingreso al BdT:
		...
		<?
			if($_GET != NULL)
			{	
				echo '<td> Número de horas disponibles
						<input type="text" name="txtHora" value="'.$rowEmp["nHoras"].'"/>
				    </td>
					<td> Inversor Activo (1 = activo; 0 = No activo)
						<input type="text" name="txtActivo" value="'.$rowEmp["activo"].'"/>
					</td>
					<td> Codigo ingreso
        ...
        <?
			...
			echo'</select>';
			?></td>
			<td> ID Asesor de la Red:
        <label>
        <? echo '<input type="text" name="txtAsesor" id="txtAsesor" value="'.$rowEmp["asesor"].'"/>';?>
        </label></td>
